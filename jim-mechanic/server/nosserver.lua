...
-- Đăng ký sự kiện sử dụng cho vật phẩm đổi màu NOS
QBCore.Functions.CreateUseableItem("noscolour", function(source, item)
    -- Khi người chơi sử dụng vật phẩm, gửi một sự kiện đến client của họ
    -- để mở menu chọn màu RGB/HEX.
    TriggerClientEvent("jim-mechanic:client:NOS:rgbORhex", source)
end)