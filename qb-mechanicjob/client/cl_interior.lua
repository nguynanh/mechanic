...
--========================================================== Interior
RegisterNetEvent('qb-mechanicjob:client:Interior:Apply', function(data)
	...
	if GetVehicleMod(vehicle, tonumber(data.bumperid)) == tonumber(data.mod) then
		...
		TriggerEvent('qb-mechanicjob:client:Interior:Choose', tonumber(data.bumperid))
	elseif GetVehicleMod(vehicle, tonumber(data.bumperid)) ~= tonumber(data.mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Interior:Choose', tonumber(data.bumperid)) end
...
RegisterNetEvent('qb-mechanicjob:client:Interior:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if GetNumVehicleMods(vehicle, 28) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label2, txt = "["..(GetNumVehicleMods(vehicle, 28)+1)..Loc[Config.Lan]["common"].menuinstalled..installed2, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 28 } } end
		if GetNumVehicleMods(vehicle, 29) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label3, txt = "["..(GetNumVehicleMods(vehicle, 29)+1)..Loc[Config.Lan]["common"].menuinstalled..installed3, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 29 } } end
		if GetNumVehicleMods(vehicle, 30) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label4, txt = "["..(GetNumVehicleMods(vehicle, 30)+1)..Loc[Config.Lan]["common"].menuinstalled..installed4, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 30 } } end
		if GetNumVehicleMods(vehicle, 31) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label5, txt = "["..(GetNumVehicleMods(vehicle, 31)+1)..Loc[Config.Lan]["common"].menuinstalled..installed5, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 31 } } end
		if GetNumVehicleMods(vehicle, 33) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label6, txt = "["..(GetNumVehicleMods(vehicle, 33)+1)..Loc[Config.Lan]["common"].menuinstalled..installed6, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 33 } } end
		if GetNumVehicleMods(vehicle, 34) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label7, txt = "["..(GetNumVehicleMods(vehicle, 34)+1)..Loc[Config.Lan]["common"].menuinstalled..installed7, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 34 } } end
		if GetNumVehicleMods(vehicle, 35) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label8, txt = "["..(GetNumVehicleMods(vehicle, 35)+1)..Loc[Config.Lan]["common"].menuinstalled..installed8, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 35 } } end
		if GetNumVehicleMods(vehicle, 36) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label9, txt = "["..(GetNumVehicleMods(vehicle, 36)+1)..Loc[Config.Lan]["common"].menuinstalled..installed9, params = { event = "qb-mechanicjob:client:Interior:Choose", args = 36 } } end
		exports['qb-menu']:openMenu(InteriorMenu)
	end
end)

RegisterNetEvent('qb-mechanicjob:client:Interior:Choose', function(mod)
	local validMods = { }
	if not outCar() then return end
	local vehicle = nil
	...
	if DoesEntityExist(vehicle) then
		...
		local ModMenu = {
				...
				{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Interior:Check" } },
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "qb-mechanicjob:client:Interior:Apply", args = { mod = -1, bumperid = tonumber(mod) } } }}
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, tonumber(mod)) == v.id then icon = "fas fa-check" disabled = true end
				ModMenu[#ModMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Interior:Apply', args = { mod = v.id, bumperid = mod } } }
			end
