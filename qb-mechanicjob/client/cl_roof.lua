...
--========================================================== Roof
RegisterNetEvent('qb-mechanicjob:client:Roof:Apply', function(mod)
	...
	if GetVehicleMod(vehicle, 10) == tonumber(mod) then triggerNotify(nil, modName..Loc[Config.Lan]["common"].already, "error") TriggerEvent('qb-mechanicjob:client:Roof:Check')
	elseif GetVehicleMod(vehicle, 10) ~= tonumber(mod) then
		...
		{ task = "WORLD_HUMAN_WELDING" }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Roof:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Roof:Check', function()
	...
	local RoofMenu = {
			...
			{ icon = icon, isMenuHeader = disabled, header = Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "qb-mechanicjob:client:Roof:Apply", args = -1 } } }
		for k,v in pairs(validMods) do
			local icon = "" local disabled = false
			if GetVehicleMod(vehicle, 10) == v.id then icon = "fas fa-check" disabled = true end
			RoofMenu[#RoofMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Roof:Apply', args = tostring(v.id) } }
		end
