...
RegisterNetEvent("qb-mechanicjob:client:CarWax", function(data)
	...
	if cleaning == false and data.time ~= 0 then
		TriggerServerEvent("qb-mechanicjob:server:WaxActivator", VehToNet(vehicle), data.time)
	end
	if Config.CosmeticRemoval and not data.skip then toggleItem(false, "cleaningkit") end
end)

RegisterNetEvent("qb-mechanicjob:client:CarWax:WaxTick", function(vehicle)
...
RegisterNetEvent('qb-mechanicjob:client:cleanVehicle', function(skip)
	...
	if DoesEntityExist(vehicle) then
		local CleanMenu = {
			...
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head1, params = { event = "qb-mechanicjob:client:CarWax", args = { time = 0, skip = skip }}}
		if Config.WaxFeatures then
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head2, params = { event = "qb-mechanicjob:client:CarWax", args = { time = 1800, skip = skip }}}
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head3, params = { event = "qb-mechanicjob:client:CarWax", args = { time = 3600, skip = skip }}}
			CleanMenu[#CleanMenu + 1] = { header = Loc[Config.Lan]["carwax"].head4, params = { event = "qb-mechanicjob:client:CarWax", args = { time = 5400, skip = skip }}}
		end
...
--Quick Repair
RegisterNetEvent('qb-mechanicjob:quickrepair', function()
...
RegisterNetEvent('qb-mechanicjob:client:openDoor', function(door)
...
RegisterNetEvent("qb-mechanicjob:flipvehicle", function()
	local playerPed	= PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	if not inCar() then	return end
	if not nearPoint(coords) then return end
...
RegisterNetEvent("qb-mechanicjob:seat", function(seat)
...
RegisterNetEvent("qb-mechanicjob:togglesound", function()
...
CreateThread(function()
	while true do
		...
		if invehicle then
			...
			if DoesEntityExist(veh) then
				if GetPedInVehicleSeat(veh, -1) == ped then
					...
					if owned then
						local p = promise.new()
						QBCore.Functions.TriggerCallback('qb-mechanicjob:distGrab', function(cb) p:resolve(cb) end, trim(GetVehicleNumberPlateText(veh)))
									...
									if not IsVehicleStopped(GetVehiclePedIsIn(ped)) then
										...
										if DistAdd >= 3000 then
											if useMechJob() then
												...
												TriggerServerEvent('qb-mechanicjob:server:saveStatus', mechDamages, plate)
											end
											TriggerServerEvent('qb-mechanicjob:server:UpdateDrivingDistance', plate, math.round(DistAdd))
											dist = dist + DistAdd
											DistAdd = 0
										end
									else Wait(5000) end
							...
							while odocalc do
								if ShowOdo and IsPedInAnyVehicle(PlayerPedId(), true) then
									...
									local lights = ""
										...
										TriggerEvent("qb-mechanicjob:client:DrawText", "<center>"..lights.."<span style=''>"..odotext.."</span>", tostring(Config.OdoLocation))
									end
								else TriggerEvent("qb-mechanicjob:client:HideText") odocalc = false Wait(1000) end
								Wait(600)
							end
						end
					end)
		...
		else TriggerEvent("qb-mechanicjob:client:HideText") end
		Wait(10000)
		if not invehicle then
			if nosCheck and VehicleNitrous[trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), true)))] then
				TriggerServerEvent('qb-mechanicjob:database:UpdateNitroLevel', trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), true))), VehicleNitrous[trim(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), true)))].level) -- Update the nos when you are no longer in that vehicle
			end
			maxspeed = 0
			nosCheck = false
			odocalc = false
			databasecalc = false
			TriggerEvent("qb-mechanicjob:client:HideText")
			DistAdd = 0
		else Wait(10000) end
	end
end)

RegisterNetEvent("qb-mechanicjob:ShowOdo", function() print("^3ShowOdo^7: ^2Odometer toggled^7") ShowOdo = not ShowOdo end)