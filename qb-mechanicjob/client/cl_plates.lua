...
--========================================================== Plates
RegisterNetEvent('qb-mechanicjob:client:Plates:Custom:Apply', function(index)
	local vehicle = getClosest(GetEntityCoords(PlayerPedId())) pushVehicle(vehicle) lookVeh(vehicle)
	if GetVehicleNumberPlateTextIndex(vehicle) == tonumber(index) then triggerNotify(nil, Loc[Config.Lan]["plates"].already, "error") TriggerEvent('qb-mechanicjob:client:Plates:Custom')
	elseif GetVehicleNumberPlateTextIndex(vehicle) ~= tonumber(index) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Plates:Custom') end
...
RegisterNetEvent('qb-mechanicjob:client:Plates:Apply', function(data)
	...
	if GetVehicleMod(vehicle, tonumber(data.mod)) == tonumber(data.id) then
		...
		TriggerEvent('qb-mechanicjob:client:Plates:Choose', data)
	elseif GetVehicleMod(vehicle, tonumber(data.mod)) ~= tonumber(data.id) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			updateCar(vehicle)
			if Config.CosmeticRemoval then toggleItem(false, "customplate")
			else TriggerEvent('qb-mechanicjob:client:Plates:Choose', data) end
			triggerNotify(nil, Loc[Config.Lan]["plates"].installed, "success")
		end, function()
			triggerNotify(nil, Loc[Config.Lan]["plates"].failed, "error")
			emptyHands(PlayerPedId())
...
RegisterNetEvent('qb-mechanicjob:client:Plates:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if GetNumVehicleMods(vehicle, 25) ~= 0 then PlatesMenu[#PlatesMenu + 1] = { header = Loc[Config.Lan]["plates"].label1, txt = "["..(GetNumVehicleMods(vehicle, 25)+1)..Loc[Config.Lan]["common"].menuinstalled..installed1, params = { event = "qb-mechanicjob:client:Plates:Choose", args = { mod = 25 } } } end
		if GetNumVehicleMods(vehicle, 26) ~= 0 then PlatesMenu[#PlatesMenu + 1] = { header = Loc[Config.Lan]["plates"].label2, txt = "["..(GetNumVehicleMods(vehicle, 26)+1)..Loc[Config.Lan]["common"].menuinstalled..installed2, params = { event = "qb-mechanicjob:client:Plates:Choose", args = { mod = 26 } } } end
		PlatesMenu[#PlatesMenu + 1] = { header = Loc[Config.Lan]["plates"].label3, txt = "["..GetNumberOfVehicleNumberPlates(vehicle)..Loc[Config.Lan]["common"].menuinstalled..installed1, params = { event = "qb-mechanicjob:client:Plates:Custom" } }

		exports['qb-menu']:openMenu(PlatesMenu)
	end
end)

RegisterNetEvent('qb-mechanicjob:client:Plates:Custom', function()
	if not inCar() then return end
	if not nearPoint(GetEntityCoords(PlayerPedId())) then return end
	local vehicle = nil
	...
	if DoesEntityExist(vehicle) then
		...
		{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Plates:Check" } } }
		for k, v in pairs(Loc[Config.Lan].vehiclePlateOptions) do
			...
			PlateMenu[#PlateMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = installed, params = { event = 'qb-mechanicjob:client:Plates:Custom:Apply', args = v.id  } }
		end
		exports['qb-menu']:openMenu(PlateMenu)
	end
end)

RegisterNetEvent('qb-mechanicjob:client:Plates:Choose', function(data)
	local validMods = { }
	if not inCar() then return end
	if not IsPedInAnyVehicle(PlayerPedId(), false) then	vehicle = getClosest(GetEntityCoords(PlayerPedId())) pushVehicle(vehicle) end
	if DoesEntityExist(vehicle) then
		...
		local ModMenu = {
				...
				{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Plates:Check" } },
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "qb-mechanicjob:client:Plates:Apply", args = { id = -1, mod = tonumber(data.mod) } } }	}
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, tonumber(data.mod)) == v.id then icon = "fas fa-check" disabled = true end
				ModMenu[#ModMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Plates:Apply', args = { id = v.id, mod = data.mod } } }
			end
