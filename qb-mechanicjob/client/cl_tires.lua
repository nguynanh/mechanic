...
--========================================================== Drift
RegisterNetEvent('qb-mechanicjob:client:applyDrift', function()
...
RegisterNetEvent('qb-mechanicjob:client:giveDrift', function()
	if GetGameBuildNumber() < 2372 then return end
	if not jobChecks() then return end
	if not locationChecks() then return end
	...
	if DoesEntityExist(vehicle) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			if GetDriftTyresEnabled(vehicle) == false then TriggerServerEvent("qb-mechanicjob:server:DupeWarn", "drifttires") emptyHands(playerPed) return end
			qblog("`drifttires - "..QBCore.Shared.Items["drifttires"].label.."` changed [**"..trim(GetVehicleNumberPlateText(vehicle)).."**]")
			SetVehicleTyreFixed(vehicle, 0)
			SetVehicleTyreFixed(vehicle, 1)
			SetVehicleTyreFixed(vehicle, 2)
...
--========================================================== Bullet Proof
RegisterNetEvent('qb-mechanicjob:client:applyBulletProof', function()
	if not jobChecks() then return end
	if not locationChecks() then return end
	if not inCar() then return end
	if not nearPoint(GetEntityCoords(PlayerPedId())) then return end
	...
	if DoesEntityExist(vehicle) then
		...
		if GetVehicleTyresCanBurst(vehicle) == false then triggerNotify(nil, Loc[Config.Lan]["common"].already2, "error") else
			...
			{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
				if GetVehicleTyresCanBurst(vehicle) == false then TriggerServerEvent("qb-mechanicjob:server:DupeWarn", "bprooftires") emptyHands(playerPed) return end
...
RegisterNetEvent('qb-mechanicjob:client:giveBulletProof', function()
	if not jobChecks() then return end
	if not locationChecks() then return end
	local vehicle = getClosest(GetEntityCoords(PlayerPedId())) pushVehicle(vehicle)
	if lockedCar(vehicle) then return end
