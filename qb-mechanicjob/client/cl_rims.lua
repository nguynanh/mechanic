...
--==========================================================  Rims
RegisterNetEvent('qb-mechanicjob:client:Rims:Apply', function(data)
	...
	{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function()  SetVehicleModKit(vehicle, 0)
		...
		if data.mod == -1 then
			...
			else TriggerEvent('qb-mechanicjob:client:Rims:Choose', data) end
		else
			if Config.CosmeticRemoval then toggleItem(false, "rims")
			else TriggerEvent('qb-mechanicjob:client:Rims:SubMenu', data) end
		end
...
RegisterNetEvent('qb-mechanicjob:client:Rims:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local WheelMenu = { }
			...
			if Config.JimMenu then WheelMenu[#WheelMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = string.gsub(Loc[Config.Lan]["common"].close, "❌ ", ""), params = { event = "qb-mechanicjob:client:Menu:Close" } }
			else WheelMenu[#WheelMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].close, params = { event = "qb-mechanicjob:client:Menu:Close" } } end
			if not cycle then
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label1, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 0, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label2, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 1, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label3, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 2, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label4, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 3, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label5, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 4, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label6, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 5, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label7, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 7, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label8, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 8, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label9, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 9, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label10, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 10, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label11, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 11, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label12, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 12, bike = false } } } end
			--if not cycle then WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label15, params = { event = "qb-mechanicjob:client:Rims:Choose", args = 6 } } end
			if cycle then
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label13, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 6, bike = false } } }
				WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label14, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = 6, bike = true } } } end
		exports['qb-menu']:openMenu(WheelMenu)
	end
end)

RegisterNetEvent('qb-mechanicjob:client:Rims:Choose', function(data)
	local playerPed	= PlayerPedId()
    local coords = GetEntityCoords(playerPed)
	local validMods = {}
	if not inCar() then return end
	...
	if not IsPedInAnyVehicle(playerPed, false) then vehicle = getClosest(coords)
		if DoesEntityExist(vehicle) then
			...
			local RimsMenu = {}
				...
				if Config.JimMenu then RimsMenu[#RimsMenu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Rims:Check" } }
				else RimsMenu[#RimsMenu + 1] =  { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "qb-mechanicjob:client:Rims:Check" } } end

				RimsMenu[#RimsMenu + 1] = { header = Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "qb-mechanicjob:client:Rims:Apply",  args = { mod = -1, wheeltype = data.wheeltype, } } }
				for k, v in pairsByKeys(validMods) do
					RimsMenu[#RimsMenu + 1] = { header = k, txt = Loc[Config.Lan]["common"].amountoption..#validMods[k], params = { event = 'qb-mechanicjob:client:Rims:SubMenu', args = { mod = v.id, wheeltype = data.wheeltype, wheeltable = validMods[k], bike = data.bike } } }
				end
...
RegisterNetEvent('qb-mechanicjob:client:Rims:SubMenu', function(data)
	local playerPed	= PlayerPedId()
    local coords = GetEntityCoords(playerPed)
	if not inCar() then return end
	if not nearPoint(coords) then return end
	...
	if not IsPedInAnyVehicle(playerPed, false) then vehicle = getClosest(coords)
		if DoesEntityExist(vehicle) then
			local RimsMenu = {
				...
				if Config.JimMenu then RimsMenu[#RimsMenu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = data.wheeltype } } }
				else RimsMenu[#RimsMenu + 1] =  { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "qb-mechanicjob:client:Rims:Choose", args = { wheeltype = data.wheeltype } } } end

				for i=1, #data.wheeltable do
					RimsMenu[#RimsMenu + 1] = { header = data.wheeltable[i].name, txt = data.wheeltable[i].install, params = { event = 'qb-mechanicjob:client:Rims:Apply', args = { mod = data.wheeltable[i].id, wheeltype = data.wheeltype, wheeltable = data.wheeltable, bike = data.bike } } }
				end
			exports['qb-menu']:openMenu(RimsMenu)
		end
	end
