...
--========================================================== Spoilers
RegisterNetEvent('qb-mechanicjob:client:Spoilers:Apply', function(mod)
	...
	if GetVehicleMod(vehicle, 0) == tonumber(mod) then
		...
		TriggerEvent('qb-mechanicjob:client:Spoilers:Check')
	elseif GetVehicleMod(vehicle, 0) ~= tonumber(mod) then
		...
		{ task = "WORLD_HUMAN_WELDING" }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Spoilers:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Spoilers:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local spoilerMenu = {
				...
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "qb-mechanicjob:client:Spoilers:Apply", args = -1 } } }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 0) == v.id then icon = "fas fa-check" disabled = true end
				spoilerMenu[#spoilerMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Spoilers:Apply', args = tostring(v.id) } }
			end
