...
RegisterNetEvent('qb-mechanicjob:client:Manual:Menu', function(data)
	if not outCar() then return end
	if Config.requireDutyCheck then
		local p = promise.new()	QBCore.Functions.TriggerCallback('qb-mechanicjob:mechCheck', function(cb) p:resolve(cb) end)
	...
	QBCore.Functions.TriggerCallback('qb-mechanicjob:checkCash', function(cb) p2:resolve(cb) end)
	...
	local RepairMenu = {}
		...
		RepairMenu[#RepairMenu+1] = { icon = "fas fa-circle-xmark", header = string.gsub(Loc[Config.Lan]["common"].close, "❌", " "), params = { event = "qb-mechanicjob:client:Menu:Close" } }
		...
		RepairMenu[#RepairMenu+1] = { isMenuHeader = greyed,
										...
										txt = settext,
										params = { event = "qb-mechanicjob:client:Manual:Repair", args = { cost = cost, society = data.society }, } }

		--RepairMenu[#RepairMenu+1] = { icon = "fas fa-magnifying-glass", header = "Preview", txt = "", params = { event = "qb-mechanicjob:client:Preview:Menu" }, }
		--RepairMenu[#RepairMenu+1] = { header = "Test", txt = "Vehicle Death Simulator", params = { event = "qb-mechanicjob:client:Police:test" }, }

	exports['qb-menu']:openMenu(RepairMenu)
end)

local repairing = false
RegisterNetEvent('qb-mechanicjob:client:Manual:Repair', function(data)
	if repairing then return end
	repairing = true
	local playerPed = PlayerPedId()
	TriggerServerEvent("qb-mechanicjob:chargeCash", data.cost, data.society)
	...
	if Config.repairAnimate then
		...
		TriggerEvent('qb-mechanicjob:client:Manual:Menu', { society = data.society })
