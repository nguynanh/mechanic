...
--========================================================== Windows
RegisterNetEvent('qb-mechanicjob:client:Windows:Apply', function(data)
	local vehicle = getClosest(GetEntityCoords(PlayerPedId())) pushVehicle(vehicle) lookVeh(vehicle)
	if lockedCar(vehicle) then return end
	if GetVehicleWindowTint(vehicle) == tonumber(data.mod) then triggerNotify(nil, data.name..Loc[Config.Lan]["common"].already, "error") TriggerEvent('qb-mechanicjob:client:Windows:Check')
	elseif GetVehicleWindowTint(vehicle) ~= tonumber(data.mod) then
		...
		{}, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Windows:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Windows:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		exports['qb-menu']:openMenu({
			...
			{ header = Loc[Config.Lan]["windows"].label1, txt = applied1, params = { event = "qb-mechanicjob:client:Windows:Apply", args = { mod = 0, name = Loc[Config.Lan]["windows"].label1 } } },
			{ header = Loc[Config.Lan]["windows"].label2, txt = applied2, params = { event = "qb-mechanicjob:client:Windows:Apply", args = { mod = 4, name = Loc[Config.Lan]["windows"].label2 } } },
			{ header = Loc[Config.Lan]["windows"].label3, txt = applied3, params = { event = "qb-mechanicjob:client:Windows:Apply", args = { mod = 5, name = Loc[Config.Lan]["windows"].label3 } } },
			{ header = Loc[Config.Lan]["windows"].label4, txt = applied4, params = { event = "qb-mechanicjob:client:Windows:Apply", args = { mod = 3, name = Loc[Config.Lan]["windows"].label4 } } },
			{ header = Loc[Config.Lan]["windows"].label5, txt = applied5, params = { event = "qb-mechanicjob:client:Windows:Apply", args = { mod = 2, name = Loc[Config.Lan]["windows"].label5 } } },
			{ header = Loc[Config.Lan]["windows"].label6, txt = applied6, params = { event = "qb-mechanicjob:client:Windows:Apply", args = { mod = 1, name = Loc[Config.Lan]["windows"].label6 } } },
		})
	end
end)