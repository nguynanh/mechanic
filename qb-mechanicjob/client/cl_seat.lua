...
--========================================================== Seat
RegisterNetEvent('qb-mechanicjob:client:Seat:Apply', function(mod)
	...
	if GetVehicleMod(vehicle, 32) == tonumber(mod) then triggerNotify(nil, modName..Loc[Config.Lan]["common"].already, "error") TriggerEvent('qb-mechanicjob:client:Seat:Check')
	elseif GetVehicleMod(vehicle, 32) ~= tonumber(mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Seat:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Seat:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local SeatMenu = {
				...
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "qb-mechanicjob:client:Seat:Apply", args = -1 } } }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 32) == v.id then icon = "fas fa-check" disabled = true end
				SeatMenu[#SeatMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Seat:Apply', args = tostring(v.id) } }
			end
