...
--========================================================== Exhaust
RegisterNetEvent('qb-mechanicjob:client:Exhaust:Apply', function(data)
	...
	if GetVehicleMod(vehicle, 4) == tonumber(data.id) then
		...
		TriggerEvent('qb-mechanicjob:client:Exhaust:Check')
	elseif GetVehicleMod(vehicle, 4) ~= tonumber(data.id) then
		...
		QBCore.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing..modName.."..", time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
			...
			else TriggerEvent('qb-mechanicjob:client:Exhaust:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Exhaust:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local ExhaustMenu = {
			...
			{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "qb-mechanicjob:client:Exhaust:Apply", args = { id = -1 } } }, }
			for k, v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 4) == v.id then icon = "fas fa-check" disabled = true end
				ExhaustMenu[#ExhaustMenu + 1] = { icon = icon, isMenuHeader = disabled,  header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Exhaust:Apply', args = { id = tostring(v.id) } } }
			end
