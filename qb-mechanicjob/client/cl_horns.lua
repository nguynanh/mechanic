...
--========================================================== Horn
RegisterNetEvent('qb-mechanicjob:client:Horn:Apply', function(data)
	...
	{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
		...
		else TriggerEvent('qb-mechanicjob:client:Horn:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Horn:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local HornMenu = {
				{ icon = "horn", header = searchCar(vehicle)..Loc[Config.Lan]["horns"].menuheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods+1, params = { event = "qb-mechanicjob:client:Horn:Test", args = { veh = vehicle } }, },
				...
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "qb-mechanicjob:client:Horn:Apply", args = { mod = -1, name = Loc[Config.Lan]["common"].stock } } } }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 14) == v.id then icon = "fas fa-check" disabled = true end
				HornMenu[#HornMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.txt, params = { event = 'qb-mechanicjob:client:Horn:Apply', args = { mod = v.id, name = v.name } } }
			end
		exports['qb-menu']:openMenu(HornMenu)
	end
end)

RegisterNetEvent('qb-mechanicjob:client:Horn:Test', function(data)
	StartVehicleHorn(data.veh, 2000, "HELDDOWN", false)
	TriggerEvent('qb-mechanicjob:client:Horn:Check')
end)