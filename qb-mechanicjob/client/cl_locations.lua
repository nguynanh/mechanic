			...
			if v.crafting then
				for l, b in pairs(v.crafting) do
					...
					exports['qb-target']:AddBoxZone("MechCraft: "..k..l, b.coords.xyz, b.w, b.d, { name="MechCraft: "..k..l, heading = b.coords.w, debugPoly=Config.Debug, minZ=b.coords.z-1.0, maxZ=b.coords.z+1.0 },
						{ options = { { event = "qb-mechanicjob:client:Crafting:Menu", icon = "fas fa-cogs", label = Loc[Config.Lan]["crafting"].menuheader, job = v.job }, },
							distance = 2.0 })
				end
			end
		end
			...
			if v.store then
				for l, b in pairs(v.store) do
					...
					exports['qb-target']:AddBoxZone("MechStore: "..k..l, b.coords.xyz, b.w, b.d, { name="MechStore: "..k..l, heading = b.coords.w, debugPoly=Config.Debug, minZ=b.coords.z-1.0, maxZ=b.coords.z+1.0 },
						{ options = { { event = "qb-mechanicjob:client:Store:Menu", icon = "fas fa-cogs", label = Loc[Config.Lan]["stores"].browse, job = v.job }, },
							distance = 2.0 })
				end
			end
		end
				...
				if v.stash then
					for l, b in pairs(v.stash) do
						...
						exports['qb-target']:AddBoxZone("MechSafe: "..k..l, b.coords.xyz, b.w, b.d, { name="MechSafe: "..k..l, heading = b.coords.w, debugPoly=Config.Debug, minZ=b.coords.z-1.0, maxZ=b.coords.z+1.0 },
							{ options = { { event = "qb-mechanicjob:client:Safe", icon = "fas fa-cogs", label = Loc[Config.Lan]["repair"].browse, job = v.job }, },
								distance = 2.0 })
					end
				end
			end
		...
		if v.manualRepair then
			for l, b in pairs(v.manualRepair) do
				...
				exports['qb-target']:AddBoxZone("RepairBench: "..k..l, b.coords.xyz, (b.w or 1.2), (b.w or 4.2), { name="RepairBench: "..k..l, heading = b.coords.w, debugPoly=Config.Debug, minZ = b.coords.z-1, maxZ = b.coords.z+1.4, },
					{ options = { { event = "qb-mechanicjob:client:Manual:Menu", icon = "fas fa-cogs", label = Loc[Config.Lan]["police"].userepair, society = v.job }, }, distance = 5.0 })
			end
		end
	...
	--Garage Locations
		if v.garage then
			...
			exports['qb-target']:AddBoxZone("MechGarage: "..k, vec3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="MechGarage: "..k, heading = out[4]+180.0, debugPoly=Config.Debug, minZ=(out.z-1.03)-0.1, maxZ=out.z-1.03+1.3 },
				{ options = { { event = "qb-mechanicjob:client:Garage:Menu", icon = "fas fa-clipboard", label = Loc[Config.Lan]["garage"].jobgarage, job = v.job, coords = v.garage.spawn, list = v.garage.list }, },
				distance = 2.0 })
		end
		...
		if v.nosrefill then
			for l, b in pairs(v.nosrefill) do
				...
				exports['qb-target']:AddBoxZone("MechNos: "..k..l, vec3(b.coords.x, b.coords.y, b.coords.z), 0.7, 0.7, { name="MechNos: "..k..l, heading = b.coords[4], debugPoly=Config.Debug, minZ=b.coords.z-1.05, maxZ=b.coords.z+0.25 },
					{ options = { { event = "qb-mechanicjob:client:NosRefill", item = "noscan", icon = "fas fa-gauge-high", label = "NOS ($"..Config.NosRefillCharge..")", coords = b.coords, tank = nos[#nos], society = v.job }, },
					distance = 2.0 })
...
----- CRAFTING STUFF -------
RegisterNetEvent('qb-mechanicjob:client:Crafting:Menu', function()
	local Menu = {}
	Menu[#Menu + 1] = { isMenuHeader = true, header = Loc[Config.Lan]["crafting"].menuheader, txt = "", }
	Menu[#Menu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = string.gsub(Loc[Config.Lan]["common"].close, "❌ ", ""), params = { event = "qb-mechanicjob:client:Menu:Close" } }
	Menu[#Menu + 1] = { header = Loc[Config.Lan]["crafting"].toolheader, txt = #Crafting.Tools..Loc[Config.Lan]["crafting"].numitems, params = { event = "qb-mechanicjob:Crafting", args = { craftable = Crafting.Tools, header = Loc[Config.Lan]["crafting"].toolheader, } } }
	Menu[#Menu + 1] = { header = Loc[Config.Lan]["crafting"].performheader, txt = #Crafting.Perform..Loc[Config.Lan]["crafting"].numitems, params = { event = "qb-mechanicjob:Crafting", args = { craftable = Crafting.Perform, header = Loc[Config.Lan]["crafting"].performheader, } } }
	Menu[#Menu + 1] = { header = Loc[Config.Lan]["crafting"].cosmetheader, txt = #Crafting.Cosmetic..Loc[Config.Lan]["crafting"].numitems, params = { event = "qb-mechanicjob:Crafting", args = { craftable = Crafting.Cosmetic, header = Loc[Config.Lan]["crafting"].cosmetheader, } } }
	exports['qb-menu']:openMenu(Menu)
end)

RegisterNetEvent('qb-mechanicjob:Crafting', function(data)
	local Menu = {}
	Menu[#Menu + 1] = { header = data.header, txt = "", isMenuHeader = true }
	Menu[#Menu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Crafting:Menu" } }
			...
			if k ~= "amount" and k ~= "job" then
				...
				if not QBCore.Shared.Items[k] then print("^5Debug^7: ^2Item not found in server^7: '^6"..k.."^7'") else
					if data.craftable[i]["job"] and hasjob == false then else
						...
						Menu[#Menu + 1] = { disabled = disable, icon = k, header = setheader, txt = settext, params = { event = "qb-mechanicjob:Crafting:MakeItem", args = { item = k, craft = data.craftable[i], craftable = data.craftable, header = data.header } } }
...
RegisterNetEvent('qb-mechanicjob:Crafting:MakeItem', function(data)
	QBCore.Functions.TriggerCallback('qb-mechanicjob:Crafting:get', function(amount)
		if not amount then triggerNotify(nil, Loc[Config.Lan]["crafting"].ingredients, 'error') else TriggerEvent('qb-mechanicjob:Crafting:ItemProgress', data) end
	end, data.item, data.craft)
end)

RegisterNetEvent('qb-mechanicjob:Crafting:ItemProgress', function(data)
	...
	{}, {}, function()
		...
		TriggerServerEvent('qb-mechanicjob:Crafting:GetItem', data.item, data.craft)
		emptyHands(PlayerPedId())
		TriggerEvent("qb-mechanicjob:Crafting", data)
	end, function() -- Cancel
...
RegisterNetEvent('qb-mechanicjob:client:NosRefill', function(data)
	...
	local p = promise.new()	QBCore.Functions.TriggerCallback('qb-mechanicjob:checkCash', function(cb) p:resolve(cb) end)
	...
	if cash >= Config.NosRefillCharge then
		...
		{}, {}, function()
			...
			TriggerServerEvent("qb-mechanicjob:chargeCash", Config.NosRefillCharge, data.society)
...
RegisterNetEvent('qb-mechanicjob:client:Store:Menu', function(data)
    exports['qb-menu']:openMenu({
        { header = Loc[Config.Lan]["stores"].tools, txt = "", params = { event = "qb-mechanicjob:client:Store", args = { id = 1, job = data.job } } },
        { header = Loc[Config.Lan]["stores"].perform, txt = "", params = { event = "qb-mechanicjob:client:Store", args = { id = 2, job = data.job } } },
        { header = Loc[Config.Lan]["stores"].cosmetic, txt = "", params = { event = "qb-mechanicjob:client:Store", args = { id = 3, job = data.job } } },
    })
end)

-- Open the selected store
RegisterNetEvent('qb-mechanicjob:client:Store', function(data)
...
------ Repair Stash Stuff -----
RegisterNetEvent('qb-mechanicjob:client:Safe', function(data) TriggerEvent("inventory:client:SetCurrentStash", data.job.."Safe") TriggerServerEvent("inventory:server:OpenInventory", "stash", data.job.."Safe", { maxweight = 4000000, slots = 50, }) end)

-------- Garage Stuff ---------
local currentVeh = { out = false, current = nil }
local garageBlip = nil

RegisterNetEvent('qb-mechanicjob:client:Garage:Menu', function(data)
	...
	if Config.JimMenu then vehicleMenu[#vehicleMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = string.gsub(Loc[Config.Lan]["common"].close, "❌ ", ""), params = { event = "qb-mechanicjob:client:Menu:Close" } }
	else vehicleMenu[#vehicleMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].close, params = { event = "qb-mechanicjob:client:Menu:Close" } } end
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		...
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = Loc[Config.Lan]["garage"].vehout,
										...
										params = { event = "qb-mechanicjob:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = Loc[Config.Lan]["garage"].remveh, params = { event = "qb-mechanicjob:client:Garage:RemSpawn" } }
	else
		...
		for k, v in pairs(data.list) do
			...
			vehicleMenu[#vehicleMenu+1] = { header = v, params = { event = "qb-mechanicjob:client:Garage:SpawnList", args = { spawnName = spawnName, coords = data.coords } } }
		end
	end
...
RegisterNetEvent("qb-mechanicjob:client:Garage:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		QBCore.Functions.SpawnVehicle(data.spawnName, function(veh)
			...
			Wait(250)
			if GetNumVehicleMods(veh, 48) > 0 or GetVehicleLiveryCount(veh) > -1 then TriggerEvent("qb-mechanicjob:client:Preview:Livery", {close=true}) end
...
RegisterNetEvent("qb-mechanicjob:client:Garage:RemSpawn", function(data)
...
RegisterNetEvent("qb-mechanicjob:client:Garage:Blip", function(data)
