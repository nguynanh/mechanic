...
--========================================================== Bumpers
RegisterNetEvent('qb-mechanicjob:client:Bumpers:Apply', function(data)
	...
	if GetVehicleMod(vehicle, tonumber(data.bumperid)) == tonumber(data.mod) then triggerNotify(nil, modName..Loc[Config.Lan]["common"].already, "error") TriggerEvent('qb-mechanicjob:client:Bumpers:Choose', tonumber(data.bumperid))
	elseif GetVehicleMod(vehicle, tonumber(data.bumperid)) ~= tonumber(data.mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Bumpers:Choose', tonumber(data.bumperid)) end
...
RegisterNetEvent('qb-mechanicjob:client:Bumpers:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if GetNumVehicleMods(vehicle, 6) ~= 0 then BumperMenu[#BumperMenu + 1] = { header = Loc[Config.Lan]["bumpers"].menugrille, txt = "["..(GetNumVehicleMods(vehicle, 6)+1)..Loc[Config.Lan]["common"].menuinstalled..installed1, params = { event = "qb-mechanicjob:client:Bumpers:Choose", args = 6 } } end
		if GetNumVehicleMods(vehicle, 1) ~= 0 then BumperMenu[#BumperMenu + 1] = { header = Loc[Config.Lan]["bumpers"].menuFBumper, txt = "["..(GetNumVehicleMods(vehicle, 1)+1)..Loc[Config.Lan]["common"].menuinstalled..installed2, params = { event = "qb-mechanicjob:client:Bumpers:Choose", args = 1 } } end
		if GetNumVehicleMods(vehicle, 2) ~= 0 then BumperMenu[#BumperMenu + 1] = { header = Loc[Config.Lan]["bumpers"].menuBBumper, txt = "["..(GetNumVehicleMods(vehicle, 2)+1)..Loc[Config.Lan]["common"].menuinstalled..installed3, params = { event = "qb-mechanicjob:client:Bumpers:Choose", args = 2 } } end
		exports['qb-menu']:openMenu(BumperMenu)
	end
end)

RegisterNetEvent('qb-mechanicjob:client:Bumpers:Choose', function(mod)
	local validMods = {}
	if not inCar() then return end
	if not nearPoint(GetEntityCoords(PlayerPedId())) then return end
	local vehicle
	...
	if DoesEntityExist(vehicle) then
		...
		local BumpersMenu = {
				...
				{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "qb-mechanicjob:client:Bumpers:Check" } }	,
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "qb-mechanicjob:client:Bumpers:Apply", args = { mod = -1, bumperid = tonumber(mod) } }	}
			}
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, tonumber(mod)) == v.id then icon = "fas fa-check" disabled = true end
				BumpersMenu[#BumpersMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Bumpers:Apply', args = { mod = tostring(v.id), bumperid = tonumber(mod) } } }
			end
