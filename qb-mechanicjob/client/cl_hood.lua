...
--========================================================== Hood
RegisterNetEvent('qb-mechanicjob:client:Hood:Apply', function(data)
	...
	if GetVehicleMod(vehicle, 7) == tonumber(data.id) then
		...
		TriggerEvent('qb-mechanicjob:client:Hood:Check')
	elseif GetVehicleMod(vehicle, 7) ~= tonumber(data.id) then
		...
		{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function() SetVehicleModKit(vehicle, 0)
			...
			else TriggerEvent('qb-mechanicjob:client:Hood:Check') end
...
RegisterNetEvent('qb-mechanicjob:client:Hood:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local HoodMenu = {
				...
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "qb-mechanicjob:client:Hood:Apply", args = { id = -1 } } }, }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleMod(vehicle, 7) == v.id then icon = "fas fa-check" disabled = true end
				HoodMenu[#HoodMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'qb-mechanicjob:client:Hood:Apply', args = { id = tostring(v.id) } } }
			end
